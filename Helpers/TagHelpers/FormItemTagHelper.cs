...
namespace MasterCore8.Helpers.TagHelpers
{
    ...
    public class FormItemTagHelper : TagHelper
    {
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            switch (Type.ToLower())
            {
                ...
                case "select":
                    ...
                    using (var html = new System.IO.StringWriter())
                    {
                        html.Write(@$"<select id='{ Id }' name='{ Name }' class='form-control {(Inline ? "form-control-inline" : "")} {(Option.Count > 10 && Select2 ? "select2" : "")} {Class}' data-name='{ Name }' data-value='{ Value }' { extraAttribute(context) } { (Required ? "required" : "") } { (Readonly ? "readonly='true'" : "") } { (Disabled ? "disabled='true'" : "") }>");
                ...
                case "signature": 
                    ...
                    string signatureBody = @$"
                        <div class=""signature-pad-main"" style=""{ (!string.IsNullOrEmpty(Width) ? "width:"+Width : "") }"">
                            ...
                            <button type=""button"" class=""signature-pad-clear btn btn-sm btn-default btn-block"" data-ref=""{Id}""> <i class=""bi bi-eraser signature-pad-clear"" data-ref=""{Id}""></i> ลบเขียนใหม่</button>
                        </div>
                    ";
        ...
        private TagBuilder CreateInput(ModelExpression _For, TagHelperContext _context){
            ...
            if(Readonly) input.Attributes["readonly"] = "true";
            if(Disabled) input.Attributes["disabled"] = "true";            
